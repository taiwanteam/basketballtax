...
function preload() {
  // 载入篮球与篮框图片
  this.load.image("background", "./assert/bc2.png");
...
function startGame(startBox, startText, startButton, reviewIcon, reviewText) {
  ...
  questionText = this.add
    .text(startPosition.x, startPosition.y - 100, "", {
      fontSize: "48px",
      fill: "#fff",
  ...
  // 建立籃球，使用物理引擎
  basketball = this.physics.add
    .image(startPosition.x, startPosition.y, "basketball")
    .setScale(0.5);
  ...
  for (let i = 0; i < 3; i++) {
    let hoop = this.add
      .image(positions[i], 250, "hoop")
      .setScale(0.5)
      .setInteractive();
    hoop.setDisplaySize(hoop.width * 0.35, hoop.height * 0.35); // 比例縮小
    hoop.answerText = this.add
      .text(positions[i], 190, "", {
        fontSize: "25px",
        fill: "#6495ED",
      })
      .setOrigin(0.5)
...
function showLoseScreen() {
  ...
  let loseText1 = this.add
    .text(770, 550, "答錯題數: " + aerror + " 題", {
      fontSize: "70px",
      fill: "#fff",
      fontFamily: "Arial, sans-serif",
    })
